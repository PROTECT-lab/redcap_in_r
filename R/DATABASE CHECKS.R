bsrc.behav_task_check<-function(boxpath= "~/OneDrive - University of Pittsburgh/PROTECT Box/data/Behavioral",
                                dnpl_path = "~/OneDrive - University of Pittsburgh/Behavioral data",
  ...
  find_snake_files<-function(task=NULL){
    path <- "~/OneDrive - University of Pittsburgh/PROTECT Box/data/Behavioral/Snake"
  ...
  snake_protectbox<-find_snake_files(task="Snake")
  ...
  find_sup_files<-function(task=NULL){
    if(task=="Grid"){
      path<-paste0(dnpl_path, "/Grid_Supp/individual_data_files")
    }
    if(task=="Helicopter"){
      path<-paste0(dnpl_path, "/Helicopter_Supp/individual_data_files")
    }
    if(task=="Twostep"){
      path<-"~/OneDrive - University of Pittsburgh/Twostep_Supp/individual_data_files"
    }
    ...
    #sup_files<-sup_files[-which(grepl("combine|test|old|Icon",sup_files))]
    #sup_file_df<-data.frame(ID=substr(sup_files, 1, 6),date=substr(sup_files, -14, -4))
    sup_file_df<-data.frame(ID=str_split(sup_files, '_', simplify = TRUE)[,2],date=substr(str_split(sup_files, '_', simplify = TRUE)[,3], 1, 10))
    ymd(sup_file_df$date)->sup_file_df$date
    #File ID doesn't match ACTUAL ID
    c("440238")->sup_file_df$ID[sup_file_df$ID %in% c("440238000000")]
  ...
  check_3_places<-function(consent=NULL, behav_data=NULL, protect_data=NULL,task=NULL){
    ...
    if(length(con>0)){
      message(paste0("This person/these people are not consented for ",task, ". Check REDcap."))
      print(con)
    }
    behav<-ids[!ids %in% behav_data]
    if(length(behav)>0 & task=="Snake"){
      message(paste0("This person/these people have no behavioral data for ",task, ". Need to download from Snake laptop; otherwise need further check why it's not there."))
      print(behav)
    }
    if(length(behav)>0 & task != "Snake"){
      message(paste0("This person/these people have no behavioral data for ",task, ". For Grid, Helicopter, Two-Step: ask Vanessa to download to the folder; otherwise need further check why it's not there."))
      print(behav)
    }
    pro<-ids[!ids %in% protect_data]
    if(length(pro)>0){
      message(paste0("This person/these people have not been updated in protect as completing ",task, ". Please go to Redcap/Protect to check 'st_whichtask___'"))
      print(pro)}
  ...
  check_3_places(consent=snake_consent,behav_data=snake_protectbox,protect_data=snake_protect, task="Snake")
